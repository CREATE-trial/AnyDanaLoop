...
class MaintenancePlugin @Inject constructor(
    ...
    fun sendLogs() {
        val recipient = sp.getString(R.string.key_maintenance_logs_email, "admin@create-trial.nz")
    ...
    fun deleteLogs() {
        LoggerUtils.getLogDirectory()?.let { logDirectory ->
            ...
            val files = logDir.listFiles { _: File?, name: String ->
                (name.startsWith("AnyDanaLoop") && name.endsWith(".zip"))
            }
    ...
    private fun constructName(): String {
        return "AnyDanaLoop_LOG_" + Date().time + LoggerUtils.SUFFIX
    }
