...
public class SafetyPlugin extends PluginBase implements ConstraintsInterface {
    ...
    public Constraint<Boolean> isLoopInvocationAllowed(@NonNull Constraint<Boolean> value) {
        ...
        if (!sp.getString(R.string.key_app_password , "").equals(Constants.APP_Password)) {//"a28ijmd$%dFGB%CXawd*"
            value.set(getAapsLogger(), false, getResourceHelper().gs(R.string.notauthorised), this);
        }
        return value;
    }
    ...
    @NonNull @Override
    public Constraint<Boolean> isFullClosedLoopAllowed(@NonNull Constraint<Boolean> value) {
        return this.isClosedLoopAllowed(value);
    }
