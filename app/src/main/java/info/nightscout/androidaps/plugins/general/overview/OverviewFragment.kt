...
class OverviewFragment : DaggerFragment(), View.OnClickListener, OnLongClickListener {
    ...
    private var commsAnimation: AnimationDrawable? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (config.PUMPCONTROL) {
            overview_temptarget?.visibility = View.GONE
            overview_activeprofile.visibility = View.GONE
            overview_apsmode.visibility = View.GONE
            overview_buttons_layout.visibility = View.GONE
            overview_as_llayout.visibility = View.GONE
            overview_chartMenuButton.visibility = View.GONE
            overview_iob_llayout.visibility = View.GONE
            overview_cob_llayout.visibility = View.GONE
            overview_basal_llayout.visibility = View.GONE
            overview_deltas_lgroup?.visibility = View.GONE
        }
        ...
        commsAnimation = overview_commsmode?.background as AnimationDrawable?
        carbAnimation?.setEnterFadeDuration(1200)
        commsAnimation?.setEnterFadeDuration(1200)
        carbAnimation?.setExitFadeDuration(1200)
        commsAnimation?.setExitFadeDuration(1200)
        ...
        overview_bggraph?.setOnLongClickListener {
            ...
            sp.putBoolean(R.string.key_stageusescale, true)
            false
        }
                ...
                R.id.overview_accepttempbutton  -> {
                    ...
                    if (loopPlugin.isEnabled(PluginType.LOOP)) {
                        ...
                        if (lastRun?.lastAPSRun != null && lastRun.constraintsProcessed?.isChangeRequested == true) {
                            OKDialog.showConfirmation(activity, resourceHelper.gs(R.string.tempbasal_label), openlooptext(lastRun.constraintsProcessed!!)
                                , Runnable {
    ...
    private fun openlooptext(data: APSResult): String {
        val units = profileFunction.getUnits()
        val rate = "%.2f".format(data.rate).toDouble()
        val iob = "%.2f".format(data.iob.iob).toDouble()
        val cob = data.COB
        var openloopsuggestiontext = "${resourceHelper.gs(R.string.setbasalquestion)}\n" +
            "$rate U/hr for ${data.duration} min \n" +
            "Reason:\n" +
            "Insulin on Board: $iob units\n" +
            "Carbohydrates on Board: $cob grams\n" +
            "Carbohydrate Ratio: ${data.cr}\n" +
            "Insulin Sensitivity: ${data.isf}\n" +
            "Glucose Target: ${Profile.toUnitsString(data.target, data.target * Constants.MGDL_TO_MMOLL, units)} $units"

        if (data.eventualBG != null)
            openloopsuggestiontext += "\nPredicted Glucose is ${Profile.toUnitsString(data.eventualBG, data.eventualBG * Constants.MGDL_TO_MMOLL, units)} $units"

        return openloopsuggestiontext
    }
    ...
    private fun updatePumpStatus(event: EventPumpStatusChanged) {
        ...
        when {
            ...
            // DISCONNECTING,
            // DISCONNECTED
            event.status == EventPumpStatusChanged.Status.CONNECTING   ->   {
                overview_commsmode?.visibility = View.VISIBLE
                commsAnimation?.start()
            }
            ...
            event.status == EventPumpStatusChanged.Status.DISCONNECTED -> {
                commsAnimation?.stop()
                carbAnimation?.selectDrawable(0)
                overview_commsmode?.visibility = View.INVISIBLE
            }
        }
    }
    ...
    private fun processButtonsVisibility() {
        ...
        if (showAcceptButton && pump.isInitialized && !pump.isSuspended && loopPlugin.isEnabled(PluginType.LOOP)) {
            ...
            overview_accepttempbutton?.text = openlooptext(lastRun?.constraintsProcessed!!)
        ...
        overview_calibrationbutton?.visibility = ((xDripIsBgSource || dexcomIsSource) && actualBG != null && sp.getBoolean(R.string.key_show_calibration_button, false)).toVisibility()
    ...
    fun updateGUI(from: String) {
        ...
        if (!profileFunction.isProfileValid("Overview")) {
            overview_status.setBackgroundColor(resourceHelper.gc(R.color.warningAlertBackground))
            overview_status.setTextColor(resourceHelper.gc(R.color.ribbonTextWarning))
            overview_status?.setText(R.string.noprofileset)
            overview_statuslayout?.visibility = View.VISIBLE
            overview_looplayout?.visibility = View.GONE
            return
        }
        overview_notifications?.let { notificationStore.updateNotifications(it) }
        overview_statuslayout?.visibility = View.GONE
        ...
        if (lastBG != null) {
            val color = when {
                lastBG.valueToUnits(units) > (highLine * 2.6)  -> resourceHelper.gc(R.color.above360)
                lastBG.valueToUnits(units) > (highLine * 2.4)  -> resourceHelper.gc(R.color.above340)
                lastBG.valueToUnits(units) > (highLine * 2.2)  -> resourceHelper.gc(R.color.above300)
                lastBG.valueToUnits(units) > (highLine * 2)    -> resourceHelper.gc(R.color.above280)
                lastBG.valueToUnits(units) > (highLine * 1.80) -> resourceHelper.gc(R.color.above260)
                lastBG.valueToUnits(units) > (highLine * 1.70) -> resourceHelper.gc(R.color.above240)
                lastBG.valueToUnits(units) > (highLine * 1.60) -> resourceHelper.gc(R.color.above200)
                lastBG.valueToUnits(units) > (highLine * 1.50) -> resourceHelper.gc(R.color.above190)
                lastBG.valueToUnits(units) > (highLine * 1.40) -> resourceHelper.gc(R.color.above180)
                lastBG.valueToUnits(units) > (highLine * 1.30) -> resourceHelper.gc(R.color.above170)
                lastBG.valueToUnits(units) > (highLine * 1.20) -> resourceHelper.gc(R.color.above160)
                lastBG.valueToUnits(units) > (highLine * 1.15) -> resourceHelper.gc(R.color.above150)
                lastBG.valueToUnits(units) > (highLine * 1.10) -> resourceHelper.gc(R.color.above140)
                lastBG.valueToUnits(units) > (highLine)        -> resourceHelper.gc(R.color.above135)
                lastBG.valueToUnits(units) > (highLine * 0.80) -> resourceHelper.gc(R.color.above125)
                lastBG.valueToUnits(units) > (highLine * 0.65) -> resourceHelper.gc(R.color.above100)
                lastBG.valueToUnits(units) > (lowLine * 1.20)  -> resourceHelper.gc(R.color.above70)
                lastBG.valueToUnits(units) > lowLine           -> resourceHelper.gc(R.color.above66)
                lastBG.valueToUnits(units) < lowLine           -> resourceHelper.gc(R.color.lower66)
        ...
        if (config.APS && pump.pumpDescription.isTempBasalCapable) {
            ...
            when {
                ...
                loopPlugin.isEnabled() && closedLoopEnabled.value() && loopPlugin.isLGS -> {
                    ...
                    overview_apsmode_text?.text = resourceHelper.gs(R.string.lowsuspend)
                }
