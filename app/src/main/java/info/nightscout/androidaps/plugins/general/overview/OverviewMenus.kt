    ...
    fun createContextMenu(menu: ContextMenu, v: View) {
        when (v.id) {
            R.id.overview_apsmode       -> {
                ...
                if (loopPlugin.isOpenLoop && constraintChecker.isFullClosedLoopAllowed().value() || loopPlugin.isLGS && constraintChecker.isFullClosedLoopAllowed().value() ) {
                    menu.add(resourceHelper.gs(R.string.closedloop))
                }
                if (!loopPlugin.isLGS && constraintChecker.isLGSAllowed().value() && loopPlugin.isEnabled(PluginType.LOOP)){
                    menu.add(resourceHelper.gs(R.string.lowglucosesuspend))
                }
    ...
    fun onContextItemSelected(item: MenuItem, manager: FragmentManager): Boolean {
        ...
        when (item.title) {
            ...
            resourceHelper.gs(R.string.lowglucosesuspend)                                    -> {
                aapsLogger.debug("USER ENTRY: LOW GLUCOSE SUSPEND ENABLED")
            ...
            resourceHelper.gs(R.string.resume), resourceHelper.gs(R.string.reconnect) -> {
                ...
                sp.putBoolean(R.string.key_stageusereconnect, true)
                loopPlugin.createOfflineEvent(0)
                return true
            }
            ...
            resourceHelper.gs(R.string.disconnectpumpfor1h)                           -> {
                ...
                sp.putBoolean(R.string.key_stageusedisconnect, true)
                rxBus.send(EventRefreshOverview("suspendmenu"))
                return true
            }
