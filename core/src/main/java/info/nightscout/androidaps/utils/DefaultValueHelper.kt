...
open class DefaultValueHelper @Inject constructor(
    ...
    /**
     * returns the corresponding Custom TempTarget based on the given units (MMOL / MGDL)
     *
     * @param units
     * @return
     */

    private fun getDefaultCustomTT(units: String): Double {
        return if (Constants.MMOL == units) Constants.defaultCustomTTmmol else Constants.defaultCustomTTmgdl
    }
    ...
    fun determineCustomTT(): Double {
        val units = profileFunction.getUnits()
        var value = sp.getDouble(R.string.key_custom_target, getDefaultCustomTT(units))
        value = Profile.toCurrentUnits(profileFunction, value)
        return if (value > 0) value else getDefaultHypoTT(units)
    }
    fun determineCustomTTDuration(): Int {
        val value = sp.getInt(R.string.key_custom_duration, Constants.defaultCustomTTDuration)
        return if (value > 0) value else Constants.defaultCustomTTDuration
    }
    /**
